...
namespace Consolation
{
    ...
    public class Console : MonoBehaviour
    {
        ...
        bool _haveFingersReleased;
        double _lastTapTime;
        double CONSECUTIVE_TAP_CUTOFF = 0.24f;
        int _touchCount = 0;
        ...
        void CheckForTwoFingerTripleTap()
		{
			...
			if (Time.time - _lastTapTime > CONSECUTIVE_TAP_CUTOFF)
			{
				_touchCount = 0;
			}

			if (!_haveFingersReleased) return;

			if (Touchscreen.current.touches.Count(t => t.press.isPressed) == 2)
			{
				_touchCount++;
				_lastTapTime = Time.time;
				_haveFingersReleased = false;
			}
			if (_touchCount == 3)
			{
				isVisible = !isVisible;
